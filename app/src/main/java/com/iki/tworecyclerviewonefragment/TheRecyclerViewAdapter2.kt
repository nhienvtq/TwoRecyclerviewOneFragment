...
class TheRecyclerViewAdapter2(private var clickListener: OnItemRV2ClickListener): RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    class RecyclerViewHolder constructor(private val context: Context, private var binding: RecyclerviewlayoutBinding):RecyclerView.ViewHolder(binding.root){
        ...
        fun onItemClicklistener(item: CardModel, action: OnItemRV2ClickListener){
            binding.pluscardView.setOnTouchListener(){ view: View, motionEvent: MotionEvent ->
                when(motionEvent.action){
                    MotionEvent.ACTION_DOWN -> binding.pluscardView.alpha = 0.5f
                    MotionEvent.ACTION_UP -> {
                        action.onItemRV2PlusClick(item, adapterPosition)
            ...
            binding.minuscardView.setOnTouchListener(){ view: View, motionEvent: MotionEvent ->
                when(motionEvent.action){
                    ...
                    MotionEvent.ACTION_UP -> {
                        action.onItemRV2MinusClick(item, adapterPosition)
...
interface OnItemRV2ClickListener{
    fun onItemRV2PlusClick(item: CardModel, position: Int)
    fun onItemRV2MinusClick(item: CardModel, position: Int)
}